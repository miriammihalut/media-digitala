...
<body class="fundal" align="center">
    ...
    <div><img /></div>
    <p style="font-size:30px; font-family: papyrus;">
        <b>Vrei sa stii ce meserie iti poti alege dupa terminarea studiilor?<br>
            ...
            <table class="cursuri">
                <tr>
                    <?php
                    $sql = "SELECT ID, titlu, domeniu, descriere FROM meserii ORDER BY ID";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        // output data of each row
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr><td><b> " . $row['titlu'] . " </b></td><td><i> " . $row['domeniu'] . " </i></td><td> " . $row['descriere'] . "</td></tr>";
                        }
                    } else {
                        echo "0 results";
                    }
